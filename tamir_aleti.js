...
console.log("==============================================================================");
console.log("             NOVA BROWSER V54 - SIBER OTOMATIK TAMIR VE YAMA ALETI            ");
console.log("==============================================================================");

const __dirname = path.resolve();

// 1. TAM YAMA: server.ts Dosyasını Kusursuz Hale Getir (HTML / API Çakışması Çözüldü)
...
app.get('/api/search', (req, res) => {
    ...
    return res.json({ success: true, query, results: [{ title: \`\${query} - NovaSearch Canli Sonucu\`, url: "https://github.com", description: "Nova Browser V54 yuksek guvenlikli yerel arama motoru altyapisi." }] });
});

app.get('*', (req, res) => { res.sendFile(path.join(__dirname, 'dist', 'index.html')); });
app.listen(PORT, () => { console.log(\`[NovaBrowser V54] Core server deployed on port \${PORT}\`); });`;

fs.writeFileSync(path.join(__dirname, 'server.ts'), serverCode, 'utf8');
console.log("[OK] server.ts dosyasındaki Rota, CORS ve HTML-API kilitleri tamamen çözüldü.");

// 2. TAM YAMA: run_hidden_server.bat Dosyasını Sıfırdan İnşa Et (Tırnak Kırılmaları Engellendi)
